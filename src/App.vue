<template>
  <div style="position:relative">
    ...
    <div class="z-index:10008; width:1000px; height:50px ; position:absolute;background:red;top:0">

      <div v-for=" (item, index) in dataHandleType" :key="index" style="width:400px" class="flex flex-wrap justify-center h-10 ml-5 text-white cursor-pointer z-1000 w-15" @click="changeHandleType(item.name, item.type)">
    ...
    <div style="z-index:10008; width:300px; height:300px ; position:absolute;background:red;bottom:0">
      ...
      <div v-for="(item,index) in newFeaturesJson" :key="index">
        ...
        <span class="flex justify-between col-span-2">
          ...
          <button style="width:50px;height:50px" @click="handleFeatureJson('edit', item)"> x</button>

          <button value="删除|svg" @click="handleFeatureJson('delete', item)">d</button>

          <button value="隐藏|svg" @click="handleFeatureJson('hidden', item)">h</button>
    ...
    <ol-map ref="map" :loadTilesWhileAnimating="true" :loadTilesWhileInteracting="true" style="height:800px">
      ...
      <ol-control-bar className="barclass">
        <ol-control-toggle :order="0" html="<i class='fas fa-map-marker'></i>" className="edit" title="Point" :onToggle="(active)=>drawEnable=false" />
        ...
        <ol-control-toggle :order="3" html="<i class='fas fa-info'></i>" className="edit" title="LineString" :onToggle="(active)=>handleClick(active,'undo')" />
        <ol-control-toggle :order="3" html="<i class='fas fa-info'></i>" className="edit" title="LineString" :onToggle="(active)=>handleClick(active,'redo')" />
      ...
      <ol-vector-layer :updateWhileInteracting="true" ref="vectorLayer" title="AIRPORTS" preview="https://raw.githubusercontent.com/MelihAltintas/vue3-openlayers/main/src/assets/tr.png">
  ...
  setup() {
      ...
      feature.setStyle(
        ...
        new Style({
          ...
          stroke: new Stroke({
            ...
            color: feature.get('color').substring(0, feature.get('color').lastIndexOf(',')) + '1)',
    ...
    const featureSelected = (event) => {
      if (event.selected.length == 1) {
        ...
        console.log('x', event.selected[0])
      } else {
